...
**Status**: Implemented (FR-01–FR-12, NFR-01–NFR-05 ✓; pending review/merge)
...
|---|---|---|
| FR-01 | The scan captures the raw pre-JavaScript HTML response body for each audited page (from the navigation response, bounded — cap at the same 500 KB order used by `tech.js` — and held in memory only for the duration of that page's processing) | ✓ Done — `createRawHtmlIndex` (500 KB cap, truncation-aware), captured in scan.js from the navigation response only when an a11y engine runs |
| FR-02 | Each finding instance (axe, Alfa, deprecated-html) gains a `render_origin: 'server' \| 'js-injected' \| 'unknown'` field, computed at scan time by testing whether the instance's element can be located in the raw HTML. Matching must be conservative: a confident structural match (not raw string equality — dynamic ids/classes and attribute reordering must not defeat it) → `server`; a confident absence of any plausible match → `js-injected`; anything ambiguous → `unknown` | ✓ Done — `classifyRenderOrigin`: structural opening-tag match, distinctive-value search (covers lazy-load attr renames), truncation can prove presence but never absence; dynamic-looking values never prove js-injected |
| FR-03 | The raw HTML body is **never** written to `data/` or `state/` — only the derived per-instance classification is persisted. (data/ is committed to git; raw bodies would bloat history and, for VA-class targets, could embed tokens — see Security rules) | ✓ Done — index is page-local scan memory; only per-example `render_origin` persists in the page record |
| FR-04 | A new pure library (`src/lib/attribution.js` or similar) classifies (a) asset URL paths and (b) element class-name namespaces into taxonomy layers, with rule tables for at minimum: Drupal (`/core/`, `/modules/contrib/`, `/modules/custom/`, `/themes/contrib/`, `/themes/custom/`, `views-`, `field--`, `block-` class namespaces), WordPress (`/wp-includes/`, `/wp-content/themes/<name>/`, `/wp-content/plugins/<name>/`, `wp-block-` namespaces), and the design-system prefixes already tabled in `component-clusters.js` (`usa-`, `ds-c-`) | ✓ Done — `src/lib/attribution.js`: `classifyAssetPath` + `classifyClassTokens` with the Drupal/WordPress/USWDS/CMS-DS tables |
| FR-05 | Namespace/path signals only support a `platform` classification when the corresponding technology is actually detected on the site by the tech engine (e.g. `views-` classes on a page where Wappalyzer did not detect Drupal must not yield `platform`); absent tech agreement the signal degrades to lower-confidence `undetermined` evidence | ✓ Done — `techConfirms` gates platform support on Wappalyzer detections; ungated matches emit null-support "not detected on this site" evidence |
| FR-06 | Aggregate derives a finding-level `attribution` object — `{ layer, confidence: 'high'\|'medium'\|'low', evidence: [{ signal, detail, supports }] }` — from the per-instance evidence, via pure functions in `src/lib/` (unit-testable with small synthetic inputs, same pattern as `progress.js`) | ✓ Done — `deriveAttribution` in `buildBugReports`; pure, covered by 23 unit tests |
| FR-07 | A finding whose instances are `js-injected` and whose element or creating context matches a third-party origin already inventoried by the third-party engine for that page is attributed `third-party`, with the origin (and Wappalyzer product name where one matches) named in the evidence | ✓ Done — implemented page-independently: the element's own URL attributes are tested with the same registrable-domain rule the third-party engine uses (`registrableDomain`), so attribution works even on pages outside that engine's sample; consulting the sampled per-page ledger was unnecessary |
| FR-08 | Findings on `<iframe>` elements embedding a third-party registrable domain are attributed `third-party` with the frame origin as evidence (note: audit engines do not descend into cross-origin frames, so this covers findings *on* the frame element, e.g. missing titles) | ✓ Done — `third-party-iframe` signal; verified by unit test (server-rendered YouTube embed → third-party) |
| FR-09 | `likely_source` is preserved for backward compatibility and derived from the new attribution (`site-custom`/`platform`/`third-party` → `template`-like spread logic retained as tie-breaker between `content` and `site-custom`); no existing consumer breaks | ✓ Done — `likely_source` preserved; attribution's `content` layer only arises at content-scale spread so values stay consistent (existing source-heuristic tests pass unchanged) |
| FR-10 | The Accessibility page shows an attribution chip per finding (layer + confidence) with the evidence list inside the finding's existing drill-down; the next-actions queue (`#h-next-actions`) groups actions under **Fix in your site** / **Report upstream** / **Third-party vendor** / **Review content** / **Undetermined** | ✓ Done — attribution chip + evidence block on the Accessibility page; next-actions queue grouped Fix in your site / Report upstream / Third-party vendor / Review content / Undetermined (headers suppressed when everything is undetermined) |
| FR-11 | The static JSON API includes `attribution` on findings additively; `schema_version` stays `"1"`; the API schema files under `src/api/schema/` are updated and `api-schema-validate` passes | ✓ Done — additive `attribution` on findings.json entries; `attribution` definition added to findings.schema.json; schema_version stays "1"; api-schema tests pass |
| FR-12 | All new user-facing strings go through `t()` and appear in the i18n template (`npm run i18n:extract` / `i18n:check` clean); severity taxonomy and internal layer keys stay English, only display labels localize | ✓ Done — labels via `t()` + dynamic-strings.json; `i18n:extract`/`i18n:check` clean; layer keys stay English internally |

## Non-Functional Requirements

| ID | Requirement | Status |
|---|---|---|
| NFR-01 | **Sustainability gate:** zero new client-side JavaScript — the chip, evidence list, and next-actions grouping are static HTML rendered at build time. No new web fonts, no external assets; any CSS addition stays within the existing budget in `src/report-html.js` | ✓ Done — chip, evidence list, and grouping are build-time static HTML; two short CSS rules added; zero client-side JS |
| NFR-02 | Scan-time overhead per page is one `response.text()` read plus in-memory matching; no additional navigation, no re-render, no second audit pass. Committed page-record growth is limited to the small per-instance classification fields | ✓ Done — one `response.text()` + in-memory matching per audited page; page records grow by one small field per kept example |
| NFR-03 | No new runtime dependency unless vendored first-party and justified in `plan.md` (repo convention; the raw-HTML matcher should prefer string/structural techniques over pulling in a full HTML parser — decision recorded in plan.md either way) | ✓ Done — no new dependency; string/structural matching, no HTML parser |
| NFR-04 | All attribution derivation lives in pure `src/lib/` functions covered by unit tests with real module APIs and synthetic inputs (no fs/db mocking); `npm run test:unit` stays green; `tests/e2e.mjs` extended for the new report surface | ✓ Done — 23 unit tests (tests/unit/attribution.test.js), full suite 466/466; e2e extended with bug-level and rendered-HTML attribution assertions, all passing |
| NFR-05 | VA (`hf_only`) handling is untouched: nothing new is persisted that the existing gitignore defenses don't already cover (guaranteed structurally by FR-03) | ✓ Done — guaranteed structurally by FR-03; nothing new persisted |
...
✓ Met — zero client-side JavaScript added; the chip, evidence list, and
next-actions grouping are build-time static HTML (two short CSS rules), and
the e2e test asserts the evidence block renders without any script. Raw HTML
bodies are processed in scan memory and discarded (FR-03).
