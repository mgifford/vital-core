...
- [x] B2. Deduplicate unchanged pages: baseline stub-dedup is wired in
  (`scan.js` write path + `aggregate.js` resolver), and prune now promotes
  retained stubs that point to a week being deleted so summaries remain
